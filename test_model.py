...
def test_mobilenet():
    ...
    train_ds, test_ds, class_names = data_load("E:\\pycharm\\project\\flower\\split_photos\\train",
                                              "E:\\pycharm\\project\\flower\\split_photos\\val", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("models/DensetNet_fv.h5")
    # model = tf.keras.models.load_model("models/MobileNet_fv.h5")
    # model.summary()
    # 测试
    loss, accuracy = model.evaluate(test_ds)
    # 输出结果
    print('densenet test accuracy :', accuracy)
    ...
    show_heatmaps(title="heatmap", x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
                  save_name="results/heatmap_densenet.png")
    # show_heatmaps(title="heatmap", x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
